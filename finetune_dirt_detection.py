...
if __name__ == "__main__":
    csv_path = Path("/Users/mariemehlfeldt/Desktop/HackMining2026/data_key_test.csv")
    df = pd.read_csv(csv_path)

    threshold_distance = 0.105 # threshold distance in meters to consider a point as "dirt"
    threshold_deriv = -1 # threshold for the first derivative of distance to consider a point as "dirt" (e.g., sudden change in distance)
    threshold_reflect = 150 # threshold reflectivity to consider a point as "dirt"

    for idx in df.index:
        filename = df.loc[idx, "filename"]
        #folder_path = Path(f"/Volumes/T7/minehack/sat_morning/{filename}")
        folder_path = Path(f"/Users/mariemehlfeldt/Desktop/Hackmingin2026_Data/rosbag2_2026_03_13-17_22_03")
        ...
        for file_ in folder_path.iterdir():
            ...
            def update(frame_idx):
                ...
                text.set_text(f"Dirty sensors: {perc_dirt:.2f}%")

                return sc, hist_line, hist_line_deriv

            #anim = FuncAnimation(fig, update, frames=len(frames_coords), interval=0)
            #plt.show()
            anim = FuncAnimation(
                fig,
                update,
            ...
            writer = FFMpegWriter(fps=30)  # fps = Geschwindigkeit des Videos
            anim.save("animation.mp4", writer=writer)
            plt.show()
        #except Exception as e:
        #    print(f"Error processing file: {file_}")
